...
const TARGET_COLLECTION = "writefromdictation";
const TASK_SUFFIX = "WFD";
...
const normalizeTaskId = (value: string) =>
  value
    .trim()
    .replace(/\s+/g, " ")
    .replace(/write\s*from\s*dictation/gi, TASK_SUFFIX)
    .toUpperCase();

const extractNumbersFromBulkText = (value: string) => {
  const strictMatches = Array.from(
    value.matchAll(/#(\d+)\s*(?:WFD|Write\s*From\s*Dictation)/gi)
  );
...
const AddAudioSamplePage: React.FC = () => {
  ...
  const extractTextFromBulk = () => {
    if (!bulkText.trim()) {
      appendMessage("Vui lòng nhập nội dung để trích xuất mã Write From Dictation.", "error");
      return;
    }
    ...
    if (numbers.length === 0) {
      appendMessage(
        "Không tìm thấy mã hợp lệ. Dùng định dạng #number WFD hoặc #number Write From Dictation.",
        "error"
      );
      return;
    }
    ...
    setBulkText("");
    appendMessage(`Đã trích xuất ${numbers.length} mã Write From Dictation.`, "success");
  };
  ...
  return (
    <AppShellBackground>
      <Head>
        <title>Add Write From Dictation Samples</title>
      </Head>
      ...
      <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-4 px-4 pb-10 pt-24 sm:gap-6 sm:px-6 lg:pt-28">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-[24px] font-bold leading-tight text-white sm:text-[30px]">
                Add Write From Dictation Samples
              </h1>
              <p className="mt-2 text-sm text-white/70 sm:text-base">
                Giao diện đồng bộ với trang Repeat Sentence, dùng cho bộ câu Write From Dictation.
              </p>
        ...
        <Card className="space-y-4">
          <h2 className="text-base font-semibold text-white sm:text-lg">Bước 1: Dán nội dung để trích xuất mã WFD</h2>
          <Input
            multiline
            value={bulkText}
            onChange={(e) => setBulkText(e.target.value)}
            placeholder="Ví dụ: #3320 WFD hoặc #3320 Write From Dictation"
            rows={6}
          />
          <div>
            <Button
        ...
        <Card className="space-y-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <Input
              ...
              placeholder="Nhập số cách nhau bởi dấu phẩy, ví dụ: 3320, 3319, 3318"
            />
            <Button
            ...
            <div className="space-y-4 rounded-card border border-rose-400/30 bg-rose-500/10 p-4">
              ...
              <div className="rounded-2xl border border-amber-300/30 bg-amber-500/10 p-4">
                ...
                <div className="mt-3 space-y-3">
                  <div>
                    <label className="mb-1 block text-xs text-white/70">Chọn mã WFD</label>
                    <select
                    ...
                    <>
                      <div>
                        <label className="mb-1 block text-xs text-white/70">
                          Nội dung câu cho #{selectedMissing} WFD
                        </label>
                        <Input
                          multiline
                          value={newContent}
                          onChange={(e) => setNewContent(e.target.value)}
                          placeholder="Nhập nội dung câu Write From Dictation..."
                          rows={3}
                        />
                      </div>
                      <Button
...
export default AddAudioSamplePage;
